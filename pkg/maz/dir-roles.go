...
func PrintAdRole(x map[string]interface{}, z *Config) {
	...
	apiUrl = ConstMgUrl + "/v1.0/directoryRoles(roleTemplateId='" + utl.Str(x["templateId"]) + "')/members"
...
func GetAzAdRoles(z *Config, verbose bool) (list []interface{}) {
	...
	apiUrl := ConstMgUrl + "/beta/roleManagement/directory/roleDefinitions"
	r, _, _ := ApiGet(apiUrl, z, nil)
	if r["value"] == nil {
		return nil
	}
