...
- Follow existing repo patterns unless an approved improvement says otherwise.
- Keep the existing single-dash option codes in `cmd/azm`, such as `-ap`, `-sfn`, and `-aprc`, as that utility's flag convention.
- Apply the short-plus-long flag pairing rule only to a new general-purpose flag such as `-v, --version`.
